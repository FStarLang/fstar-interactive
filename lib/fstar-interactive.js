...
module.exports = {
  ...
  activate: function(state) {
    ...
    atom.commands.add('atom-workspace', 'fstar-interactive:trigger',        this.checkTillCurrentCursor.bind(this));
    atom.commands.add('atom-workspace', 'fstar-interactive:gotoandtrigger', this.findMarkerAndCheck.bind(this));
  ...
  handleCursorChange: function(ev) {
    var self = this
      ...
      self.errorMarkers.forEach(function(m) {
        if(m.marker.getBufferRange().containsPoint(ev.newBufferPosition)) {
          ...
          self.currentDecoration =
            atom.workspace.getActiveTextEditor().decorateMarker
  ...
  checkTillCurrentCursor: function() {
    this.checkSelection(false);
  },

  findMarkerAndCheck: function() {
    this.checkSelection(true);
  },

  //The main function
  checkSelection: function(find) {
    this.stdout = new Buffer(0);
    this.stderr = new Buffer(0);
    this.errors = [];
    if (this.errorMarkers) {
  ...
  highlightErrors : function () {
    ...
    if (!base) return;
    var editor = atom.workspace.getActiveTextEditor();
    // this is the editor we will track cursor changes for
    if(this.cursorChangeListener === undefined) {
      this.cursorChangeListener = editor.onDidChangeCursorPosition
